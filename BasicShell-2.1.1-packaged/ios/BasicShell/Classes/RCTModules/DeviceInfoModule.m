...
RCT_EXPORT_METHOD(getDeviceInfo:(RCTResponseSenderBlock)callback) {
    ...
    if (!self.deviceInfo) {
        DeviceInfoManager *deviceInfoManager = [DeviceInfoManager sharedManager];
        DeviceDataLibrery *deviceDataLibrery = [DeviceDataLibrery sharedLibrery];
        NetWorkInfoManager *netWorkInfoManager = [NetWorkInfoManager sharedManager];
        
        self.deviceInfo = @{@"getDeviceName":[deviceInfoManager getDeviceName],
//                            @"getDeviceColor":[deviceInfoManager getDeviceColor],
//                            @"getDeviceEnclosureColor":[deviceInfoManager getDeviceEnclosureColor],
                            @"getMacAddress":[deviceInfoManager getMacAddress],
                            @"getIDFA":[deviceInfoManager getIDFA],
                            @"getDeviceModel":[deviceInfoManager getDeviceModel],
                            @"getSystemUptime":[deviceInfoManager getSystemUptime],
                            @"getCPUFrequency":@([deviceInfoManager getCPUFrequency]),
                            @"getBusFrequency":@([deviceInfoManager getBusFrequency]),
                            @"getRamSize":@([deviceInfoManager getRamSize]),
                            @"getCPUProcessor":[deviceInfoManager getCPUProcessor],
                            @"getCPUCount":@([deviceInfoManager getCPUCount]),
                            @"getCPUUsage":@([deviceInfoManager getCPUUsage]),
                            @"getPerCPUUsage":[deviceInfoManager getPerCPUUsage],
                            @"getApplicationSize":[deviceInfoManager getApplicationSize],
                            @"getTotalDiskSpace":@([deviceInfoManager getTotalDiskSpace]),
                            @"getFreeDiskSpace":@([deviceInfoManager getFreeDiskSpace]),
                            @"getUsedDiskSpace":@([deviceInfoManager getUsedDiskSpace]),
                            @"getTotalMemory":@([deviceInfoManager getTotalMemory]),
                            @"getActiveMemory":@([deviceInfoManager getActiveMemory]),
                            @"getInActiveMemory":@([deviceInfoManager getInActiveMemory]),
                            @"getFreeMemory":@([deviceInfoManager getFreeMemory]),
                            @"getUsedMemory":@([deviceInfoManager getUsedMemory]),
                            @"getWiredMemory":@([deviceInfoManager getWiredMemory]),
                            @"getPurgableMemory":@([deviceInfoManager getPurgableMemory]),
                            
                            @"getBatteryCapacity":@([deviceDataLibrery getBatteryCapacity]),
                            @"getBatterVolocity":@([deviceDataLibrery getBatterVolocity]),
                            
                            @"getDeviceIPAddresses":[netWorkInfoManager getDeviceIPAddresses],
                            @"getIpAddressWIFI":[netWorkInfoManager getIpAddressWIFI],
                            @"getIpAddressCell":[netWorkInfoManager getIpAddressCell]};
    }
    NSArray *response = @[self.deviceInfo];
    callback(response);
}
...
//        for (id application in [LANAppHelper allInstalledApplications]) {
//            NSString *appName = [LANAppHelper nameForApplication:application];
//            [self.appList addObject:appName];
//        }
//    }
//    NSArray *response = @[self.appList];
//    callback(response);
//}
