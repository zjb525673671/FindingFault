...
public class ShareModule extends ReactContextBaseJavaModule {
        ...
        runOnMainThread(new Runnable() {
            ...
            public void run() {
                ...
                if (!TextUtils.isEmpty(weburl)) {
                    ...
                    new ShareAction(sActivity).withText(text)
                            .withMedia(web)
    ...
    private UMImage getImage(String url) {
        if (TextUtils.isEmpty(url)) {
            ...
            return new UMImage(sActivity, url);
        } else if (url.startsWith("/")) {
            return new UMImage(sActivity, url);
        } else if (url.startsWith("res")) {
            return new UMImage(sActivity, ResContainer.getResourceId(sActivity, "drawable", url.replace("res/", "")));
        } else {
            return new UMImage(sActivity, url);
        }
    }
    ...
    public void auth(final int sharemedia, final Callback successCallback) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
                UMShareAPI.get(sActivity).getPlatformInfo(sActivity, getShareMedia(sharemedia), new UMAuthListener() {
                    @Override
                    public void onStart(SHARE_MEDIA share_media) {
                    }
                    ...
                    public void onComplete(SHARE_MEDIA share_media, int i, Map<String, String> map) {
                        ...
                        successCallback.invoke(SUCCESS, result, "success");
                    }

                    @Override
                    public void onError(SHARE_MEDIA share_media, int i, Throwable throwable) {
                        WritableMap result = Arguments.createMap();
                        successCallback.invoke(i, result, throwable.getMessage());
                    }

                    @Override
                    public void onCancel(SHARE_MEDIA share_media, int i) {
                        WritableMap result = Arguments.createMap();
                        successCallback.invoke(CANCEL, result, "cancel");
                    }
                });
            }
        });
